...
public:
    ...
    Member(const string _name, const int _age) : name(_name), age(_age) { memberCount++; }
...
ostream& operator<<(ostream& os, const Member& member) {
    os << "Name: " << member.name << ", Age: " << member.age << ", Level: " << member.level<<", Type: "<<typeid(member).name() ;
    return os;
}
...
istream& operator>>(istream& is, Member& member) {
    ...
    shared_ptr<Member> newMember;
    switch (type) {
        case 1:
            newMember = static_pointer_cast<Member>(make_shared<Executioner>(name, age));
            break;
        case 2:
            newMember = static_pointer_cast<Member>(make_shared<Healer>(name, age));
            break;
        case 3:
            newMember = static_pointer_cast<Member>(make_shared<Leader>(name, age));
            break;
        default:
            cout << "Invalid member type." << endl;
            return is;
...
public:
    Trybe() : trybe_name("Default") {
        trybe_id++;
    }
    
    Trybe(string name) : trybe_name(name) {
        trybe_id++;
    }
    
    Trybe(const Trybe& other) :  trybe_name(other.trybe_name) {
...
public:
    ...
    m.setLevel(newLevel);
    cout << m.getName() << "'s level increased to " 
    << newLevel << "!" << endl;
    }
};
...
public:
    ...
    void updateAnyTrybeInfo() {
        ...
        for (int i = 0; i < allTrybes.size(); i++) {
        ...
        if (choice < 1 || choice > allTrybes.size()) {
    ...
    void riot() {
        ...
        bool chosen = false;
        while (!chosen) {
            if (firstTrybeIndex < 1 || firstTrybeIndex > activeTrybes.size()) {
        ...
        while (!chosen) {
            if (secondTrybeIndex < 1 || secondTrybeIndex > activeTrybes.size() || 
        ...
        vector<bool> trumpCard;
        trumpCard.push_back(false);
        trumpCard.push_back(false);
        while (battleContinues) {
            ...
            while (memberIndex < 1 || memberIndex > attackingMembers.size()) {
            ...
            if (healer) {
                ...
                while (targetIndex < 1 || targetIndex > attackingMembers.size()) {
                    cout << "Invalid choice. Choose from (1-" << attackingMembers.size() << "): ";
                    cin >> targetIndex;
                }
                ...
                for (int i = 0; i < defendingMembers.size(); i++) {
                ...
                while (targetIndex < 1 || targetIndex > defendingMembers.size()) {
                ...
                else cout<<"trump card already used!!";
            }
            
            updateTrybe(activeTrybes);
             if (!(activeTrybes[firstTrybeIndex]->getMembers().empty() or activeTrybes[secondTrybeIndex]->getMembers().empty()))
             {
...
int main() {
    ...
    Repository<Item> sack("Sack of Magical Items");
    
    sack.add(&a1);
    sack.add(&a2);
    sack.add(&t);
    
    cout << "\n=== Items in the Sack ===" << endl;
    sack.printAll();
